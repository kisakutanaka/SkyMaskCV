 ...
 */
export function downsample2x(src) {
 ...
 */
export function blueDominance(src) {
...
export function boxBlur(field, width, height, radius) {
    ...
    for (let x = 0; x < width; x++) {
        let sum = 0;
        for (let k = -radius; k <= radius; k++)
            sum += tmp[clamp(k, height) * width + x];
        for (let y = 0; y < height; y++) {
            out[y * width + x] = sum / n;
 ...
 */
export function localStdDev(field, width, height, radius) {
    const sq = new Float32Array(field.length);
    for (let i = 0; i < field.length; i++)
        sq[i] = field[i] * field[i];
 ...
 */
export function morphSquare(mask, width, height, radius, mode) {
 ...
 * （`drawImage`）に任せる。
 */
export function upscaleNearest(mask, srcW, srcH, dstW, dstH) {
 ...
 */
export function estimateSkySeed(blue, gray, tex, width, height, opts) {
    ...
    if (blues.length < opts.minSeedCount)
        return null;
 ...
 */
export function classifyBySeed(blue, gray, tex, seed, opts) {
 ...
 */
export function keepTopComponent(mask, width, height, minAreaRatio) {
    ...
    for (let seed = 0; seed < width; seed++) {
        ...
        while (head < tail) {
            ...
            if (p >= width && mask[p - width] && !visited[p - width]) {
                visited[p - width] = 1;
                queue[tail++] = p - width;
            }
            if (p < n - width && mask[p + width] && !visited[p + width]) {
                visited[p + width] = 1;
                queue[tail++] = p + width;
            }
        }
 ...
 */
export function refineBoundary(fine, coarse, coarseW, coarseH, seed, opts) {
    ...
    for (let i = 0; i < out.length; i++)
        out[i] = fineBand[i] ? fresh[i] : fineBase[i];
    return out;
}
 ...
 */
export function segmentSky(fine, options = {}) {
    ...
    if (seed === null)
        return new Uint8Array(fine.width * fine.height);
 ...
 */
export function despeckle(mask, width, height) {
    const out = new Uint8Array(width * height);
    for (let y = 1; y < height - 1; y++) {
        const row = y * width;
        for (let x = 1; x < width - 1; x++) {
            const i = row + x;
            const sum = mask[i - width - 1] + mask[i - width] + mask[i - width + 1] +
